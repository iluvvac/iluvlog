                    ...
                    <div className="md:col-span-7 p-8 md:p-12 flex flex-col justify-center">
                      <div className="flex gap-3 mb-6">
                        {featuredPost.tags?.map((tag) => (
                          <span key={tag.value} className="text-[10px] font-bold text-purple-600 bg-purple-50 px-2 py-0.5 border border-purple-100">
                            #{tag.label}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="mt-auto pt-6 border-t border-slate-50 flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase">
                          ...
                          last_modified: {new Date(featuredPost.publishedAt).toLocaleDateString("en-GB").toUpperCase()}
                        </div>
...
async function getData() {
  ...
  const content = page ? await markdownToHtml(page.content) : "// No introduction content found.";
