...
export default async function Index() {
  ...
  const wallpaperUrl =
    "https://images.unsplash.com/photo-1644426358812-879f02d1d867?q=80&w=2728&auto=format&fit=crop";

  const myTools: ToolItem[] = [
    {
      title: "ASCII Text Pattern Generator",
            ...
            <div className="w-72 bg-[#fff9c4] rounded-lg shadow-sm border border-black/5 flex flex-col transition-all hover:rotate-1 rotate-[-1deg] duration-500">
              ...
              <div className="p-6">
                <h2 className="text-xl font-bold tracking-tight text-slate-800 leading-tight mb-4">
                  ...
                  <br />
                  <span className="text-purple-600 italic">design</span>, and
                  the <br />
          ...
          <div className="w-full max-w-5xl h-[92vh] md:h-[85vh] bg-white rounded-2xl shadow-lg flex flex-col border border-black/5 overflow-hidden">
            ...
            <div className="flex flex-1 overflow-hidden">
              <aside className="w-48 bg-[#f3f3f3]/80 p-5 hidden md:block border-r border-slate-200 overflow-y-auto">
                <p className="text-[10px] font-bold text-slate-400 uppercase mb-4 tracking-wider px-2">
                  Favorites
                </p>
                <nav className="space-y-0.5 mb-10 text-[13px]">
                  <Link href="/">
                    <div className="bg-slate-200 text-slate-900 px-3 py-1.5 rounded-lg font-semibold flex items-center gap-2 cursor-pointer transition-colors">
                      🏠 Home
                    </div>
                  </Link>
                ...
                <nav className="space-y-0.5 mb-10 text-[13px]">
                  <Link href="/tools/ascii-text-pattern">
                    <div className="text-slate-600 px-3 py-1.5 hover:bg-slate-200/50 rounded-lg flex items-center gap-2 transition-colors cursor-pointer">
                      🔤 ASCII Text Pattern
                    </div>
                  </Link>
                ...
                <div className="space-y-0.5 px-1 text-[12px]">
                  {allTags.map((tag) => (
                    <div
                      key={tag}
                      className="text-slate-600 px-3 py-1.5 hover:bg-slate-200/50 rounded-md flex items-center gap-2 cursor-pointer transition-colors capitalize"
                    >
                            ...
                            <div className="flex-1 flex flex-col gap-2 md:gap-4">
                              <div className="flex flex-wrap items-center gap-2">
                                {featuredPost.tags?.slice(0, 1).map((tag) => (
                                  <span
                                    key={tag.value}
                                    className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-purple-400 bg-purple-900/30 px-2 py-0.5 rounded border border-purple-500/30 backdrop-blur-sm"
                                  >
                                    {tag.label}
                                  </span>
                                ))}
...
async function getData() {
  ...
  const allTags = Array.from(
    new Set(allPosts.flatMap((p) => p.tags?.map((t) => t.label) || []))
  );
