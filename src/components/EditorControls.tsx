...
const ColorPickerItem = ({ value, onChange, label, align = 'left' }: { value: string, onChange: (val: string) => void, label?: string, align?: 'left' | 'right' }) => {
    ...
    <div className="flex flex-col gap-1 w-full relative" ref={popoverRef}>
      ...
      <div className="flex items-center bg-[#1c1c1e] border border-white/10 rounded-lg focus-within:border-blue-500 focus-within:ring-1 transition-all h-8">
        ...
        <input
          ...
          maxLength={7}
          className="bg-transparent text-white text-xs w-full min-w-0 px-2 outline-none uppercase font-mono"
        />
      </div>
      {isOpen && (
        <div className={`absolute z-[100] top-[calc(100%+8px)] ${align === 'right' ? 'right-0' : 'left-0'} bg-[#2c2c2e] p-3 rounded-xl border border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-150`}>
...
export default function EditorControls({ state, setters, canvasRef }: any) {
  ...
  return (
    <div className="w-full md:w-80 flex-1 md:flex-none md:h-auto bg-[#1e1e1e]/80 p-4 md:p-5 flex flex-col gap-6 overflow-y-auto border-t md:border-t-0 md:border-r border-white/5 custom-scrollbar shrink-0 order-2 md:order-1 relative z-50">
      ...
      <div className="bg-[#2c2c2e] p-4 rounded-xl border border-white/5 flex flex-col gap-4 shadow-sm">
        ...
        {state.colorMode === 'original' ? (
          ...
          <div className={`grid gap-3 pt-2 ${state.colorMode === 'duotone' ? 'grid-cols-2' : 'grid-cols-1'}`}>
            <ColorPickerItem label="Primary" value={state.colorOne} onChange={(val) => setters.setColorOne(val)} align="left" />
            {state.colorMode === 'duotone' && (
              <ColorPickerItem label="Secondary" value={state.colorTwo} onChange={(val) => setters.setColorTwo(val)} align="right" />
            )}
          </div>
        )}
      </div>
      ...
      <div className="bg-[#2c2c2e] p-4 rounded-xl border border-white/5 flex flex-col gap-3 shadow-sm">
        ...
        {state.bgMode === 'solid' && (
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="w-28">
              <ColorPickerItem value={state.bgColor} onChange={(val) => setters.setBgColor(val)} align="right" />
            </div>
          </div>
        )}
